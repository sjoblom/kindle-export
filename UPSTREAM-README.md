...
- [Usage](#usage)
  ...
  - [(Optional) Extract Illustrations](#optional-extract-illustrations)
...
6. (Optional) Run `src/extract-illustrations.ts` ([details](#optional-extract-illustrations))
...
### (Optional) Extract Illustrations

Run the illustration extractor:

```sh
ASIN=B0819W19WD pnpm extract:illustrations
```

The script uses a two-stage pipeline:

1. **OpenCV Pre-screening**: Fast, local computer vision detects candidate non-text regions and rejects pages that are text-heavy.
2. **GPT Confirmation**: If candidates are found, it analyzes page screenshots with `gpt-5.2-mini`, confirms candidates by semantic class (`diagram`, `chart`, `decorative`, etc.), and keeps only informative ones.

It writes:

- `out/${asin}/illustrations/<index>-<page>-<illusIdx>.png`
- `out/${asin}/illustrations.json`
- `out/${asin}/illustrations.skipped.json` (validation / skip reasons - great for decision logs)
...
ASIN=B0819W19WD pnpm extract:illustrations -- \
  --keep-score-threshold 0.8 \
  --opencv-min-area-percent 4 \
  --opencv-max-text-density 0.7 \
  --enable-post-crop-verifier

# balanced (default)
ASIN=B0819W19WD pnpm extract:illustrations -- \
  --keep-score-threshold 0.7 \
  --opencv-min-area-percent 2 \
  --opencv-max-text-density 0.8

# lenient (captures more, may include noise)
ASIN=B0819W19WD pnpm extract:illustrations -- \
  --keep-score-threshold 0.5 \
  --opencv-min-area-percent 1 \
  --opencv-max-text-density 0.95
```

Helpful QA/debug options:

```sh
# run a small sample page set first with debug logging
ASIN=B0819W19WD pnpm extract:illustrations -- \
  --page-indices "5,25,80,140" \
  --overwrite \
  --trim-white-border \
  --opencv-debug
```
...
- _(This should run instantly)_
- The result is stored to `out/${asin}/${short_book_title}.md`.
- The filename is lowercased, underscore-separated, and capped at 80 characters.
- If `out/${asin}/illustrations.json` exists, extracted illustrations are embedded inline in the markdown.

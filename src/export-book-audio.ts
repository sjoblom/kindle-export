...
By ${authors.join(', ')}`
  ...
  // A preview is a single page, so there are no real section boundaries to
  // resolve — narrate all of it under the first TOC entry.
  const sections = isPreview
    ? metadata.toc
        .filter((tocItem) => tocItem.page !== undefined)
        .slice(0, 1)
        .map((tocItem) => ({ tocItem, chunks: content, nextLabel: undefined }))
    : resolveBookSections(metadata.toc, content)

  for (const { tocItem, chunks, nextLabel } of sections) {
    // Aggregate the text. Headings stay plain paragraphs so they're narrated
    // rather than read out as markdown.
